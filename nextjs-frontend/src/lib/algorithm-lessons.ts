  ...
  return {
    ...
    dijkstra: {
      ...
      steps: expandedSteps('dijkstra', [
        ...
        { title: 'Initialize all distances', action: 'Set every tentative distance to infinity.', reason: 'No route has been discovered yet.', visual: 'All non-source nodes are muted.', state: [['dist[*]', '∞']], takeaway: 'Infinity is replaced only by an explicit route.', lines: [1], renderPhase: 0 },
        { title: 'Seed the source', action: `Set ${data.nodes[dijkstra.source].short} to distance 0.`, reason: 'The empty route has zero cost.', visual: 'The source turns amber and grows.', state: [['source', data.nodes[dijkstra.source].short], ['distance', '0']], takeaway: 'Dijkstra expands outward from one source.', lines: [1], renderPhase: 0 },
        { title: 'Initialize the min-heap', action: 'Push the source with priority zero.', reason: 'The heap must expose the smallest tentative distance.', visual: 'A source label appears beside the active node.', state: [['heap', `[(0, ${dijkstra.source})]`]], takeaway: 'Heap priority controls settlement order.', lines: [2], renderPhase: 0 },
        { title: 'Inspect an outgoing weighted edge', action: `Consider ${data.nodes[dijkstra.source].short} → ${data.nodes[8].short}.`, reason: 'Relaxation tests whether this edge improves its target.', visual: 'The inspected edge brightens.', state: [['edge probability', String(Math.exp(-(dijkstra.distances[8] ?? 0)).toFixed(3))]], takeaway: 'Each directed edge proposes one candidate distance.', lines: [5], renderPhase: 1 },
        { title: 'Calculate a candidate distance', action: 'Add settled distance and transformed edge cost.', reason: 'Path costs add under the log transformation.', visual: 'Candidate distance appears by the target.', state: [['candidate', String(dijkstra.distances[8])]], calculation: `0 + ${dijkstra.distances[8]} = ${dijkstra.distances[8]}`, takeaway: 'Relaxation is a concrete arithmetic comparison.', lines: [5, 6], renderPhase: 1 },
        { title: 'Accept an improving relaxation', action: `Update ${data.nodes[8].short} and set its parent to the source.`, reason: 'The candidate is smaller than infinity.', visual: 'The target and parent edge activate.', state: [['dist[8]', String(dijkstra.distances[8])], ['parent[8]', String(dijkstra.parent[8])]], takeaway: 'Parents preserve the route that produced each best distance.', lines: [6], renderPhase: 1 },
        { title: 'Extract the closest unsettled node', action: 'Remove the minimum-distance heap entry.', reason: 'Non-negative costs make this distance final.', visual: 'The closest node becomes saturated.', state: [['closest non-source', `${data.nodes[8].short}: ${dijkstra.distances[8]}`]], takeaway: 'A settled node never needs a shorter distance later.', lines: [3, 4], renderPhase: 1 },
        { title: 'Reject non-improving candidates', action: 'Keep an existing distance when a candidate is not smaller.', reason: 'Only strict improvements belong in the heap.', visual: 'Rejected edges return to muted styling.', state: [['update condition', 'candidate < dist[v]']], takeaway: 'Not every inspected edge changes the shortest-path tree.', lines: [6], renderPhase: 1 },
        { title: 'Settle all reachable species', action: 'Repeat extract-min and relaxation until the heap is empty.', reason: 'Every reachable node eventually receives a final distance.', visual: 'Finite-distance nodes color by settlement distance.', state: [['finite', String(dijkstra.distances.filter(value => value !== null).length)], ['unreachable', String(dijkstra.distances.filter(value => value === null).length)]], takeaway: 'Disconnected species remain infinite.', lines: [3, 4, 5, 6], renderPhase: 1 },
        { title: 'Reconstruct ESKAPE paths', action: 'Follow parent pointers backward from each reachable target.', reason: 'Distances give cost; parents give an explanatory route.', visual: 'Gold arrows reveal target paths.', state: reachableEskape.map(path => [path.target_name, `p=${path.probability}`] as [string, string]), takeaway: 'A shortest-path result should be interpretable as a sequence of transfers.', lines: [7], renderPhase: 2 },
        { title: 'Convert cost back to risk', action: 'Compute exp(-distance) and interpret the strongest links.', reason: 'Probability is more intuitive than log-distance.', visual: 'The highest-risk edge is emphasized.', state: [['strongest', `${dijkstra.highest_risk.src_name} → ${dijkstra.highest_risk.tgt_name}`], ['probability', String(dijkstra.highest_risk.probability)]], calculation: 'probability = exp(-distance)', takeaway: 'The result is the most probable modeled route, not a guaranteed biological event.', lines: [8], renderPhase: 3 },
      ]),
      pseudocode: [
        { line: 1, code: 'dist[source] <- 0; dist[others] <- infinity' },
        { line: 2, code: 'push (0, source) into min-heap' },
        { line: 3, code: 'while heap is not empty:' },
        { line: 4, code: '  (du, u) <- extract-min' },
        { line: 5, code: '  for each edge u -> v with cost -log(weight):' },
        { line: 6, code: '    if du + cost < dist[v]: update dist[v], parent[v], heap' },
        { line: 7, code: 'reconstruct target paths through parent pointers' },
        { line: 8, code: 'probability <- exp(-distance)' },
      ],
      ...
      resultInterpretation: `${reachableEskape.length} ESKAPE targets are reachable from ${dijkstra.source_name}. The strongest edge in the full graph is ${dijkstra.highest_risk.src_name} to ${dijkstra.highest_risk.tgt_name} at probability ${dijkstra.highest_risk.probability}.`,
    },
    floyd_warshall: {
      id: 'floyd_warshall',
    ...
    greedy_contain: {
      ...
      steps: expandedSteps('greedy_contain', [
        ...
        { title: 'Collect candidate directed edges', action: `Gather all ${data.meta.n_edges} graph edges.`, reason: 'Any directed transfer link could be removed.', visual: 'All edges remain visible.', state: [['candidates', String(data.meta.n_edges)]], takeaway: 'Reciprocal directions are separate candidates.', lines: [2], renderPhase: 0 },
    ...
    bnb_contain: {
      ...
      steps: expandedSteps('bnb_contain', [
        ...
        { title: 'Initialize an incumbent cut', action: `Use a feasible solution as the current best bound.`, reason: 'Any branch with equal or greater cost can then be pruned.', visual: 'The search tree root appears.', state: [['incumbent', String(bnb.greedy_cost)]], takeaway: 'A good incumbent makes pruning stronger.', lines: [1], renderPhase: 1 },
        { title: 'Choose the next undecided edge', action: 'Select a candidate edge for a binary decision.', reason: 'Every subset can be represented by remove/keep branches.', visual: 'The next tree level appears.', state: [['decision', 'remove edge or keep edge']], takeaway: 'Branching systematically covers the solution space.', lines: [2, 5], renderPhase: 1 },
        ...
        { title: 'Evaluate a lower bound', action: 'Estimate the best possible completion of the partial branch.', reason: 'The bound decides whether improvement remains possible.', visual: 'Bound values appear beside tree nodes.', state: [['prune rule', 'lower bound ≥ incumbent']], takeaway: 'Bounds accelerate exhaustive search without changing correctness.', lines: [3], renderPhase: 1 },
        ...
        { title: 'Test a complete cut', action: 'Run reachability after all required decisions are made.', reason: 'Only source-target disconnection makes a branch feasible.', visual: 'The graph shows whether protected targets remain connected.', state: [['feasibility test', 'all targets disconnected']], takeaway: 'A low-cost branch is irrelevant if it fails containment.', lines: [4], renderPhase: 1 },
        { title: 'Improve the incumbent', action: `Store a feasible ${bnb.optimal_cost}-edge cut.`, reason: 'It beats the previous best solution.', visual: 'The optimal search path turns green.', state: [['new incumbent', String(bnb.optimal_cost)]], takeaway: 'Every improved incumbent enables additional pruning.', lines: [4], renderPhase: 2 },
        { title: 'Exhaust all competitive branches', action: 'Continue until every unvisited branch is safely pruned or evaluated.', reason: 'Only then is the incumbent provably optimal.', visual: 'The search tree shows explored and pruned regions.', state: [['optimality', 'certified']], takeaway: 'No unexplored branch can contain a better solution.', lines: [7], renderPhase: 2 },
        { title: 'Compare exact and greedy costs', action: `Compare B&B ${bnb.optimal_cost} with greedy ${bnb.greedy_cost}.`, reason: 'The same subgraph makes the quality gap meaningful.', visual: 'A side-by-side result panel appears.', state: [['B&B', String(bnb.optimal_cost)], ['greedy', String(bnb.greedy_cost)], ['saved', String(bnb.greedy_cost - bnb.optimal_cost)]], calculation: `${bnb.greedy_cost} - ${bnb.optimal_cost} = ${bnb.greedy_cost - bnb.optimal_cost}`, takeaway: 'Branch and bound supplies the minimality certificate that greedy lacks.', lines: [8], renderPhase: 3 },
      ]),
      pseudocode: [
      ...
      spaceComplexity: 'O(E)',
      limitations: ['Worst-case runtime is exponential.', 'The exact certificate applies only to the selected hospital subgraph.', 'Bound quality determines practical runtime.'],
      resultInterpretation: `Branch and bound proves that ${bnb.optimal_cost} directed removals are sufficient and minimal on the hospital subgraph. The stored greedy run removes ${bnb.greedy_cost}, a gap of ${bnb.greedy_cost - bnb.optimal_cost} edges.`,
    },
  }
}
